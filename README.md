...
bottleneck of the code: on a 500-zone Type II model a full run takes
**164.5 min**, almost all of it inside the linear solve.
...
| ------ | ----- | --------- | ------- |
| Y12M (stock) | 1 | 164.5 min | 1.0x |
| PARDISO | 1 | 78.3 min | 2.10x |
| PARDISO | 2 | 71.7 min | 2.29x |
| PARDISO | 3 | 69.4 min | 2.37x |

![Runtime comparison](docs/runtime_comparison.png)

The bolometric light curve agrees with the stock code to within a few percent,
with the largest differences at isolated shock zones:

![Lightcurve comparison](docs/lightcurve_comparison.png)
...
A worked end-to-end example - building the code, writing the
shell-plus-wind Type IIn inputs, the `.hyd`/`.abn`/`.dat` field
meanings, and running - is in
[docs/run_a_type_iin_model.ipynb](docs/run_a_type_iin_model.ipynb).

`examples/typeiin/` has a self-contained version of that chain:
...
`eve2.exe` -> `xronfict.exe` -> `xstella.exe`. Neither depends on
...
The curves lie on top of each other: the median difference is
~0.002 dex (~0.5%) and the worst single-zone difference (~0.08 dex)
sits on the shock-front spike, where a one-zone offset on a steep
gradient inflates the pointwise metric.

![Profile agreement](validation/validation_profiles.png)

Full 15-band light curve from the 1-CPU PARDISO run:
