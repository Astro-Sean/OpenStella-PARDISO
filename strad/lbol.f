     ...
     *ubvR,LubvI,Lyman
      ...
      write(*,*)'runname=',runname
      lrun=len_trim(runname)
      write(*,*)' lrun=',lrun
      write(*,*)'runname=',runname(1:lrun)
      ...
      write(*,*)filestr
      L=1
09999 IF(.NOT.(CS*1.D+08/(freqmn(L)*Ufreq)>Rlong))GOTO09998
      L=L+1
      GOTO09999
09998 CONTINUE
      Lb=L
      Write(*,'(1p,3(a,e12.5))')' lammn(L)=',CS*1.D+08/(freqmn(L)*Ufreq)
     *,' Rlong=',Rlong,' lam(L)=',CS*1.D+08/(freq(L)*Ufreq)
09996 IF(.NOT.(CS*1.D+08/(freqmn(L)*Ufreq)>Ushort))GOTO09995
      L=L+1
      GOTO09996
09995 CONTINUE
      Lf=L-1
      Write(*,'(1p,3(a,e12.5))')' lammn(Lf)=',CS*1.D+08/(freqmn(Lf)*Ufre
     *q),' Ushort=',Ushort,' lam(Lf+1)=',CS*1.D+08/(freq(Lf+1)*Ufreq)
      write(51,*)'   time          L_ubvri    L_bol    XEUV<325   IR>890
     *'
09993 CONTINUE
      read(50,' (F15.10,1X,I4,1X,E10.3,130F8.3)' ,end=10)t_tt,NFRUS,dumm
     *y,(ttt(L),L=1,NFREQ)
      SUMbol=0.
      DO09990L=1,min(NFRUS,Nfreq)
      SUMbol=SUMbol+10.**ttt(L)*(freq(L+1)-freq(L))*Ufreq
09990 CONTINUE
      WRITE(*,*)' Rest observer Lum=',SUMbol
      SUMubvri=0.
      DO09987L=Lb,Lf
...
09981 CONTINUE
      write(51,'(F15.10,5F10.3)')t_tt,log10(sumUBVRI),log10(sumbol),log1
     *0(SUMXEUV),log10(SUMfarIR)
      GOTO09993
10    close(50)
      close(51)
      end
