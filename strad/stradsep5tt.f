     ...
     *d0-(exp(-(FREQMN(Lbl)/Tpbl))))**2
      ...
      GOTO09997
09999 CONTINUE
      write(*,*)' using runname from strad.1 file'
      GOTO09997
09998 CONTINUE
      write(*,*)' using runname from argument'
...
09987 CONTINUE
09984 IF(.NOT.(TAU(max(Ncnew,1))<=FitTau.AND.Ncnew>=NCNMIN))GOTO09983
      Ncnew=Ncnew-1
      GOTO09984
09983 CONTINUE
      WRITE(*,'(2I8,I3,3(A,1P,G10.3),A,I3,A,G11.2)')NSTEP,MAXER,NQUSED,'
     * Hu=',HUSED,' Ro=',Rho(1),' td=',tday,' Ncnd=',Ncnd,' Tau=',TAU(ma
     *x(Ncnd,1))
      IF(Ncnew<NCNMIN)Ncnew=0
      IF(Ncnew>0.AND.Ncnew<Ncnd-NCN_JUMP)Ncnew=Ncnd-NCN_JUMP
      Ncnew=MIN(Ncnew,NZON-3)
      HOLDFR=.TRUE.
      IF(.NOT.((TAU(max(Ncnd,1))>TauTol*FitTau.or.TAU(max(Ncnd,1))*TauTo
     *l<FitTau).AND.Ncnew/=Ncnd.AND.(CHNCND.or.Nstep==0)))GOTO09981
      IF(.NOT.(Ncnew<Ncnd))GOTO09978
      WRITE(4,'(2(A,I4),2(A,1P,E10.2))')' Ncnew=',Ncnew,'  Ncnd=',Ncnd,'
     * TAU(Ncnd)=',TAU(Ncnd),'   TAU(Ncnew)=',TAU(max(Ncnew,1))
      WRITE(*,'(2(A,I4),2(A,1P,E10.2))')' Ncnew=',Ncnew,'  Ncnd=',Ncnd,'
     * TAU(Ncnd)=',TAU(Ncnd),'   TAU(Ncnew)=',TAU(max(Ncnew,1))
      DO09974L=1,Nfrus
...
09968 CONTINUE
09965 IF(.NOT.(BLACK(MIN(LFR+1,NFREQ),Tpcomp)*FREQMN(MIN(LFR+1,NFREQ))**
     *3>FLNOIS))GOTO09964
      LFR=LFR+1
      IF(.NOT.(LFR>=NFREQ))GOTO09965
09964 CONTINUE
      LFR=min(LFR,NFREQ)
      IF(LFR/=NFRUS)THEN
      WRITE(4,*)'Tpcomp=',Tpcomp
      WRITE(4,*)' NEW NFRUS -->',LFR,' AT STEP=',NSTEP
      WRITE(*,*)' NEW NFRUS -->',LFR,' AT STEP=',NSTEP
      ENDIF
...
09938 CONTINUE
      ...
      CALLOPACIT
      CAP2=CAPPA
      WRITE(*,'(2(A,I4),2(A,1P,E10.2))')' before ADD NEW FJ:  Ncnew=',Nc
     *new,'  Ncnd=',Ncnd,' TAU(Ncnd)=',TAU(Ncnd),'   TAU(Ncnew)=',TAU(ma
     *x(Ncnew,1))
...
09932 CONTINUE
      ...
      callHAPPA
      CAP2=CAPPA
      FLLF1=((Ry(K)*TP1)**4-(Ry(K)*TP)**4)*TP1**4/(CAP1*(DM(K)+DM(K+1))*
     *(TP1**4+TP**4))
      FLRT1=((Ry(K)*TP1)**4-(Ry(K)*TP)**4)*TP**4/(CAP2*(DM(K)+DM(K+1))*(
     *TP1**4+TP**4))
      FLUM=CLUM*(FLLF1+FLRT1)
      Endif
      If(NSTEP==0.AND.EddJ(K,LubvB)>0.25D0)Then
      ...
      If(K<Nzon)Then
      If(Nstep>1.AND.K>Nthick(L))then
      Y(IX+(K-Ncnew)+KRAD1,1)=FradH(K,L)
      elseif(Nstep>1)then
      Y(IX+(K-Ncnew)+KRAD1,1)=(FLLF1+FLRT1)*Blackd(L,0.5D0*(Tp+Tp1))*CAP
     *2/(6.D0*(0.5D0*(Tp+Tp1))**4*Ry(K)**2*HAPPAL(L))
      else
...
09929 CONTINUE
      ...
      WRITE(4,'(2(A,1P,E12.5),A,I3)')' Cond Lum=',Flum,'      New  Lum='
     *,Flun,'   Km=',k
      Endif
09935 CONTINUE
      Ncnd=Ncnew
      KRAD=KRAD1
      N=NZON*NVARS+2*KRAD
      write(*,*)' NEW Ncnd KRAD N:   ',Ncnd,KRAD,N,' AT STEP=',NSTEP
      WRITE(4,*)' NEW Ncnd KRAD N:   ',Ncnd,KRAD,N,' AT STEP=',NSTEP
      HOLDFR=.FALSE.
      GOTO09975
09978 CONTINUE
      IF(.NOT.(Ncnew>Ncnd.AND.Ncnew<=NZON-3))GOTO09977
      WRITE(4,'(2(A,I4),2(A,1P,E10.2))')' Ncnew=',Ncnew,'  Ncnd=',Ncnd,'
     * TAU(Ncnd)=',TAU(max(Ncnd,1)),'   TAU(Ncnew)=',TAU(max(Ncnew,1))
      WRITE(0,'(2(A,I4),2(A,1P,E10.2))')' Ncnew=',Ncnew,'  Ncnd=',Ncnd,'
     * TAU(Ncnd)=',TAU(max(Ncnd,1)),'   TAU(Ncnew)=',TAU(max(Ncnew,1))
      DO09926L=1,Nfrus
...
09920 CONTINUE
      ...
      Y(NVARS*NZON+(NZON-Ncnew)*(L-1)+(K-Ncnew)+KRAD1,1)=Y(IX+(K-Ncnd)+K
     *RAD,1)
09911 CONTINUE
09914 CONTINUE
      trlx=25.d0*h
...
09905 CONTINUE
      ...
      Write(*,*)' Tpcomp   Ycomp:  ',Tpcomp,Ycomp
      LFR=NFRUS
      write(*,*)Tpcomp,LFR
09902 IF(.NOT.(BLACK(LFR,max(Tpcomp,floor(3)))*FREQMN(LFR)**3<FLNOIS/1.6
     *D0.AND..NOT.HOLDFR.AND.Lfr>NFRMIN))GOTO09901
...
09899 CONTINUE
      ...
      CallNTH(Ncnd)
      KTAIL=-50
      ELSEIF(LFR<NFREQ)THEN
09893 IF(.NOT.(BLACK(LFR+1,max(Tpcomp,floor(3)))*FREQMN(LFR+1)**3>FLNOIS
     *))GOTO09892
      LFR=LFR+1
      IF(.NOT.(LFR==NFREQ))GOTO09893
09892 CONTINUE
      IF(LFR/=NFRUS)THEN
      WRITE(4,*)'Tpcomp=',Tpcomp
      KRAD1=(NZON-Ncnd)*LFR
...
09890 CONTINUE
      ...
      IX=NVARS*NZON+(NZON-Ncnd)*(L-1)
      Y(IX+(K-Ncnd),1)=Y(IX+(K-Ncnd)-(NZON-Ncnd),1)*EXP((FREQMN(L-1)-FRE
     *QMN(L))/Tpcomp)
...
09884 CONTINUE
      ...
      NEEDBR=.TRUE.
      WRITE(4,*)' NEW NFRUS KRAD N-->',NFRUS,KRAD,N,' AT STEP=',NSTEP
      WRITE(*,*)' NEW NFRUS KRAD N-->',NFRUS,KRAD,N,' AT STEP=',NSTEP
      CallNTH(Ncnd)
      KTAIL=-50
      ENDIF
      ENDIF
...
09878 CONTINUE
      ...
      Endif
      If(Lsystem)then
      CALLDATTIM(D0001,CT001)
      else
      callDateTime(D0001,CT001)
      endif
      WRITE(4,'(1X,''   ====> STEP:'',I6,2X,A,2X,'' ON  '',A)')NSTEP,CT0
     *01,D0001
      CALLFeau(Ncnd,Ncnd)
      trlx=25.d0*h
      write(*,*)trlx
...
09866 CONTINUE
      ...
      I=NZON
      TAU(I)=0.D0
09863 IF(.NOT.(I>1.AND.I>=Ncnd))GOTO09862
      TP=Ty(I)
      PL=3.D0*DM(I)/(Ry(I)**3-Ry(I-1)**3)
      DO09860ii=1,Natom
      Yat(ii)=YABUN(ii,I)
...
09857 CONTINUE
      ...
      TOO=TO(1)
      KO=nmom
09854 IF(.NOT.(KFLAG==0.and.NSTEP<NSTMAX))GOTO09853
      doI=1,N
      Ysave(I)=Y(I,1)
      enddo
      doIFL=1,NVARS
      doI=1,NZON
      YMAX(I+(IFL-1)*NZON)=MAX(ABS(Y(I+(IFL-1)*NZON,1)),FLOOR(IFL))*Wacc
     *(IFL)
      enddo
      enddo
      DO09851IFL=1,Nfrus
      ...
      YMAX(I+Nvars*Nzon+(IFL-1)*(NZON-Ncnd))=MAX(ABS(Y(I+Nvars*Nzon+(IFL
     *-1)*(NZON-Ncnd),1)),FLOOR(Nvars+1))
      else
      YMAX(I+Nvars*Nzon+(IFL-1)*(NZON-Ncnd))=MAX(ABS(Y(I+Nvars*Nzon+(IFL
     *-1)*(NZON-Ncnd),1)),FLOOR(Nvars+1)/(5.d0*Freqmn(IFL)**3))
      endif
09848 CONTINUE
09851 CONTINUE
...
09838 CONTINUE
      istim=2
09836 IF(.NOT.(stmlog(istim)<tdlog))GOTO09835
      istim=istim+1
      GOTO09836
09835 CONTINUE
09837 CONTINUE
...
09818 IF(.NOT.(Badste))GOTO09817
      Badste=.false.
      K=1
09815 IF(.NOT.(.not.Badste.and.K<=Nzon))GOTO09814
      Ry(K)=Y(K,1)
      Uy(K)=Y(Nzon+K,1)
      if(K>1)then
      if(Ry(K)<=Ry(K-1))then
      write(*,*)' BADSTE stradsep5tt Km=',K-1,'  R0=',Ry(K-1),'  u0=',Uy
     *(K-1)
      ...
      Ty(K)=Y(2*Nzon+K,1)
      if(Ty(K)<=0.d0)then
      write(*,*)' stradsep5tt: bad step - T, Km=',K,'  R1=',Ry(K),'  u1=
     *',Uy(K),'  Ty=',Ty(K)
      Badste=.true.
      endif
      K=K+1
...
09812 CONTINUE
      ...
      write(18,'(a,1P,4E12.4)')' k=max(Ncnd,1)    L=40 FJ Bb: ',Y(NVARS*
     *NZON+(NZON-Ncnd)*(40-1)-Ncnd+max(Ncnd,1),1),Black(40,Ty(max(Ncnd,1
     *)))
      write(18,'(a,1P,4E12.4)')' k=Ncnd+1 L=40 FJ Bb: ',Y(NVARS*NZON+(NZ
     *ON-Ncnd)*(40-1)+1,1),Black(40,Ty(Ncnd+1))
      Endif
      RX=1.D0
      IF(HUSED/=HOLDBL)THEN
      DO09809J=2,NQused+1
      RX=RX*(HUSED/HOLDBL)
...
09800 CONTINUE
      IF(IABS(JSTART)>NQUSED)YENTOT(NQused+1+1)=ERLUM*EL(NQused+1)/FLOAT
     *(NQused+1)
      HOLDBL=HUSED
      CallNTH(Ncnd)
      write(*,*)' Ltau=',Ltau
...
09796 CONTINUE
      ...
      tau(K)=tau(K)-tau(Ncnew)
09788 CONTINUE
09785 IF(.NOT.(TAU(max(Ncnew,1))<=FitTau.AND.Ncnew>=NCNMIN))GOTO09784
      Ncnew=Ncnew-1
      GOTO09785
09784 CONTINUE
      WRITE(*,'(2I8,I3,3(A,1P,G10.3),A,I3,A,G11.2)')NSTEP,MAXER,NQUSED,'
     ...
     *x(Ncnd,1))
      ...
      HOLDFR=.TRUE.
      IF(.NOT.((TAU(max(Ncnd,1))>TauTol*FitTau.OR.TAU(max(Ncnd,1))*TauTo
     *l<FitTau).AND.Ncnew/=Ncnd.AND.(CHNCND.OR.Nstep==0)))GOTO09782
      IF(.NOT.(Ncnew<Ncnd))GOTO09779
      WRITE(4,'(2(A,I4),2(A,1P,E10.2))')' Ncnew=',Ncnew,'  Ncnd=',Ncnd,'
     * TAU(Ncnd)=',TAU(Ncnd),'   TAU(Ncnew)=',TAU(max(Ncnew,1))
      WRITE(*,'(2(A,I4),2(A,1P,E10.2))')' Ncnew=',Ncnew,'  Ncnd=',Ncnd,'
     * TAU(Ncnd)=',TAU(Ncnd),'   TAU(Ncnew)=',TAU(max(Ncnew,1))
...
09775 CONTINUE
     ...
     * TAU(Ncnd)=',TAU(Ncnd),'   TAU(Ncnew)=',TAU(max(Ncnew,1))
      ...
      If(Ty(I)>Tpcomp)Tpcomp=Ty(I)
09769 CONTINUE
09766 IF(.NOT.(BLACK(MIN(LFR+1,NFREQ),Tpcomp)*FREQMN(MIN(LFR+1,NFREQ))**
     *3>FLNOIS))GOTO09765
      LFR=LFR+1
      IF(.NOT.(LFR>=NFREQ))GOTO09766
09765 CONTINUE
      LFR=min(LFR,NFREQ)
      IF(LFR/=NFRUS)THEN
      WRITE(4,*)'Tpcomp=',Tpcomp
...
09736 CONTINUE
      ...
      GOTO09776
09779 CONTINUE
      IF(.NOT.(Ncnew>Ncnd.AND.Ncnew<=NZON-3))GOTO09778
      WRITE(4,'(2(A,I4),2(A,1P,E10.2))')' Ncnew=',Ncnew,'  Ncnd=',Ncnd,'
     * TAU(Ncnd)=',TAU(max(Ncnd,1)),'   TAU(Ncnew)=',TAU(max(Ncnew,1))
      WRITE(0,'(2(A,I4),2(A,1P,E10.2))')' Ncnew=',Ncnew,'  Ncnd=',Ncnd,'
     * TAU(Ncnd)=',TAU(max(Ncnd,1)),'   TAU(Ncnew)=',TAU(max(Ncnew,1))
...
09782 CONTINUE
      If(KFLAG==0)then
      NTRANS=NZON
09709 IF(.NOT.(TAU(NTRANS)<3.D-4.AND.NTRANS>Ncnd+1))GOTO09708
      NTRANS=NTRANS-1
      GOTO09709
09708 CONTINUE
      NTRANS=NZON-NTRANS
      IF(.NOT.(Nzon>Nzmin.AND.NTRANS>=20.AND.T*Utime>TREZON))GOTO09706
      NDELZN=NTrans
      WRITE(4,'('' ==> Dropping '',I3,'' zones at STEP:'',I5)')Ndelzn,Ns
     *tep
      WRITE(*,'('' ==> Dropping '',I3,'' zones at STEP:'',I5)')Ndelzn,Ns
     *tep
...
09700 CONTINUE
      ...
      Y(NVARS*Nznew+(Nznew-Ncnd)*(L-1)+(K-Ncnd)+KRAD1,1)=Y(IX+(K-Ncnd)+K
     *RAD,1)
09691 CONTINUE
09694 CONTINUE
      Nzon=Nznew
...
09682 CONTINUE
      LFR=NFRUS
      write(*,*)Tpcomp,LFR
09679 IF(.NOT.(BLACK(LFR,max(Tpcomp,floor(3)))*FREQMN(LFR)**3<FLNOIS/1.6
     *D0.AND..NOT.HOLDFR.AND.Lfr>NFRMIN))GOTO09678
      write(*,*)Tpcomp,LFR,floor(3),FREQMN(LFR)
      LFR=LFR-1
      GOTO09679
09678 CONTINUE
...
09676 CONTINUE
      ...
      KTAIL=-50
      ELSEIF(LFR<NFREQ)THEN
09670 IF(.NOT.(BLACK(LFR+1,max(Tpcomp,floor(3)))*FREQMN(LFR+1)**3>FLNOIS
     *))GOTO09669
      LFR=LFR+1
      IF(.NOT.(LFR==NFREQ))GOTO09670
09669 CONTINUE
      IF(LFR/=NFRUS)THEN
      WRITE(4,*)'Tpcomp=',Tpcomp
      KRAD1=(NZON-Ncnd)*LFR
...
09655 CONTINUE
      If((NSTEP-KTAIL>=50.OR.MOD(NSTEP,MBATCH)==0.OR.MOD(NSTEP,50)==0))t
     *hen
      Ryzer=Ry(Kmcor)
      RTphi(Nzon+1)=0.d0
      EpsUq=0.d0
...
09643 CONTINUE
      ...
      KTAIL=NSTEP
      Ycomp=(Boltzk*UTp*Tpcomp)/(Amelec*Cs**2)
      Write(*,*)' Tpcomp   Ycomp after Feau:  ',Tpcomp,Ycomp
      write(*,*)' 1st Jstart=',Jstart
      NEEDBR=.TRUE.
      Evalja=.TRUE.
      endif
      If(JSTART==0)then
      ...
      tretard=Ry(NZON-1)/CLIGHT
      t_ob=(t-tretard)*UTIME
09637 IF(.NOT.(t_ob>=TOO.AND.KO<=KOMAX.AND.Nstep>10))GOTO09636
...
09636 CONTINUE
     ...
     *01,D0001
      ...
      if(Nstep>1)CALLPRIMAG
      endif
      IF(KFLAG==0)THEN
      If(t*UTIME>=8.64D4*tcurB)NSTMAX=NSTEP
      If((MOD(NSTEP,MBATCH)==0.OR.NSTEP==NSTMAX).AND..NOT.Givdtl)then
      CALLSAVEM
      If(Lsystem)then
      CALLDATTIM(D0001,CT001)
      else
      ...
      WRITE(4,'(1X,''   ====> SAVED:'',I6,2X,A,2X,'' ON  '',A)')NSTEP,CT
     *001,D0001
...
09622 CONTINUE
      ...
      GOTO09618
09621 CONTINUE
      Write(6,*)' corrector iterations fail to converge for',' H=HMIN: K
     *FLAG=',KFLAG
      Write(*,*)' corrector iterations fail to converge for',' H=HMIN: K
     *FLAG=',KFLAG
      GOTO09618
09620 CONTINUE
...
09618 CONTINUE
     ...
     *01,D0001
      ...
      Isys=SYSTEM('mv  '//runname(1:lrun)//'.ph'//' $HOMEStella/res/')
      Isys=SYSTEM('mv  '//runname(1:lrun)//'.tt'//' $HOMEStella/res/')
      Isys=SYSTEM('mv  '//runname(1:lrun)//'.swd'//' $HOMEStella/res/')
      Isys=SYSTEM('mv  '//runname(1:lrun)//'.lbol'//' $HOMEStella/res/')
      Isys=SYSTEM('mv  '//runname(1:lrun)//'.res'//' $HOMEStella/res/')
