...
C- 3 - NOCONV,4 - CONV
      ...
      PARAMETER(Mzon=500)
C-  PARAMETER(Mzon=500); -- h toy model
C- PARAMETER(Mzon=500); -- zoning for W7
C- PARAMETER(Mzon=500); -- zoning for W7+wind
C-PARAMETER(Mzon=500); -- zoning for W7fhh+wind
C-PARAMETER(Mzon=500); -- zoning for W7fhh
C-   PARAMETER (Mzon=500); -- zoning for ntomi Crab model
...
C- PARAMETER(NYDIM=2*(@Npe+@Npc)*Mzon,MAXDER=4); -- to use in Feau
      Parameter(Is=5)
C- for test -- chego???
C-   PARAMETER (NZ=1200000); --  for Nfreq=200, Mzon=500
      PARAMETER(NZ=3000000)
C-  for Nfreq=100, Mzon=500
...
C: Vars for GPLOT    *
      ...
      DATATHD/' Tp  Rho  P  S       ','           Spectra   ','         
     *      FIG. 3','               FIG. 4'/
      DATATHX/'          Zone Number','        Freq. group Number','    
     * Lagr. mass (Solar units)','        Radius (1.E14 cm)'/
      DATATHY/'              LG(Tp D P) ','              log( J) ','    
     *           LOG(RPH/RSUN)','                   -MV'/
...
C- full print with graphics
      Fullp=Ipout<=0
C- full print
      write(*,*)' ipout fullgp,p ',ipout,Fullgp,Fullp
...
C: print head_line : stiff & head of zone output *
      WRITE(4,'(''%H:'')')
      WRITE(4,'(''   NSTEP   KFLAG  JSTART  NQUSED    NFUN    NJAC'',   
     *           ''       N   NFrus    Ncnd'',              ''   NITER  
     * NFAIL   NZMOD'')')
      WRITE(4,'(12I8)')NSTEP,KFLAG,JSTART,NQUSED,NFUN,NJAC,N,NFrus,Ncnd,
     *NITER,NFAIL,NZM
      WRITE(4,'(''  OBS.TIME='',F15.5,'' D   PROPER T='',1P,E15.8,'' S''
     *,              ''  STEP USED='',E11.5,              ''  STEP TRIED
     *='',E11.5,'' S'')')(tout-tretard)*(UTIME*CRAP)/86400.D0,tout*(UTIM
     *E*CRAP),HUSED*(UTIME*CRAP),H*(UTIME*CRAP)
      If(CONV)Then
...
09993 CONTINUE
      PL=RHO(Km)
      If(Ty(km)<0)write(*,*)' Ty(km)=',Ty(km),'  in km=',km
C-         Tp=abs(Ty(Km));
      Tp=Ty(Km)
C-         CHEM=CHEM0(Km);
      DO09990ii=1,Natom
...
C-    Km=Km-1; -- wrong place???
      ...
      Endif
      If(tau(Km)>=0.64D0.AND.IRph==0)IRph=Km
C-  @wterm ' km pl @freqtau hap ',km,pl,@freqtau,happal(@freqtau);
      Km=Km-1
C- new place
      IF(.NOT.(Km==0.OR.(tau(Km+1)>=0.64D0)))GOTO09993
C-   _UNTIL Km==0 ! (tau(Km) >= @tauPH ! tauTp >= @tauPH);
      IRph=min(IRph,Nzon-2)
      JPH=IRph
      If(NCND>0)Then
C: find Rph, Vph & Tph interpolating in tau *
      RPH=Ry(IRph+1)+(Ry(IRph)-Ry(IRph+1))/(tau(IRph)-tau(IRph+1))*(0.64
     *D0-tau(IRph+1))
      RPH=max(RPH,Ry(1))
      VPH=Uy(IRph+1)+(Uy(IRph)-Uy(IRph+1))/(tau(IRph)-tau(IRph+1))*(0.64
     *D0-tau(IRph+1))
      XPH=YABUN(1,IRph+1)+(YABUN(1,IRph)-YABUN(1,IRph+1))/(tau(IRph)-tau
     *(IRph+1))*(0.64D0-tau(IRph+1))
C-   0.5D0*(tau(IRph)+tau(IRph+1)) -- tau IN THE MIDDLE OF ZONE IRph+1
      TPH4=Ty(IRph+2)**4+(Ty(IRph+1)**4-Ty(IRph+2)**4)/(0.5D0*(tau(IRph)
     *-tau(IRph+2)))*(0.64D0-0.5D0*(tau(IRph+1)+tau(IRph+2)))
      TPH=ABS(TPH4)**.25D0
C-   TPH=SIGN(TPH,TPH4);
      else
...
C- CHANGE!!! FOR FTEFF
      ...
      write(*,*)' Lbalrad RPH=',rph
      I=1
09984 IF(.NOT.(I<NZON-1.AND.Ry(I+1)<RPH))GOTO09983
      I=I+1
      GOTO09984
09983 CONTINUE
C- Ry(I+1)>=RPH
...
C- tday
      write(*,*)' Jph, Km, NCND *******',Jph,km,ncnd
09981 IF(.NOT.(Km>0))GOTO09980
...
C-      @wterm' km rho pl: ',Km,Rho(KM), Pl;
      ...
      kmhap=Km
      CALLHAPPA
      write(40,'(1x,i4,1p,2e10.2,0p,(20f6.2))')km,pl*urho,Tp*uTp,(log10(
     *max(1.d-50,hapabs(ll)/(ur*urho))),ll=1,nfrus)
      If(Scat)write(40,'('' scat'',20x,20f6.2)')(log10(max(1.d-50,(happa
     *l(ll)-hapabs(ll))/(ur*urho))),ll=1,nfrus)
      CallOpacit
      Press(Km)=P
...
C-        _ Km<=NCND
C-             [
      FLLF1=((Ry(Km)*Tp1)**4-(Ry(Km)*Tp2)**4)*Tp1**4/(CAP1*(DM(Km)+DM2)*
     *(Tp1**4+Tp2**4))
      FLRT1=((Ry(Km)*Tp1)**4-(Ry(Km)*Tp2)**4)*Tp2**4/(CAP2*(DM(Km)+DM2)*
     *(Tp1**4+Tp2**4))
      FL1=FLLF1+FLRT1
C- ]
...
C-                /(DM(Km)+DM(Km+1));
      HAPHL=(Tp**4+Ty(Km+1)**4)/(Ty(Km+1)**4*Uplim/(WORK(Km+1,L)+Haplim*
     *WORK(Km,L))+Tp**4/WORK(Km,L))
C-           Accrad=Accrad+Y(@NFHL+Km,1)*WEIGHT(L)*HAPHL;
      Accrad=Accrad+Yout(NVARS*NZON+(NZON-NCND)*(L-1)-NCND+KRAD+Km)*WEIG
     *HT(L)*HAPHL
      else
C-           Flum=Flum+Y(@NFHL+Km-KRAD,1)*WEIGHT(L)*HEDD(L);
      Flum=Flum+Yout(NVARS*NZON+(NZON-NCND)*(L-1)-NCND+KRAD+Km-KRAD)*WEI
     *GHT(L)*HEDD(L)
      HAPHL=WORK(NZON,L)
C-           Accrad=Accrad+Y(@NFHL+Km-KRAD,1)*WEIGHT(L)*HAPHL*HEDD(L);
      Accrad=Accrad+Yout(NVARS*NZON+(NZON-NCND)*(L-1)-NCND+KRAD+Km-KRAD)
     **WEIGHT(L)*HAPHL*HEDD(L)
      endif
09963 CONTINUE
...
C- ETERM=ETERM+EGAS*DM(Km)*UEI;
      ...
      IF(Km/=1.AND.Km/=NCND+1)EVIR=EVIR+Press(km)*UPI*(DM(Km)/Rho(km))
...
C-  XCARB=ACARB*YCARB;
      ...
      AMPR=AM(Km)*UM
      If(AMPR<1.01*(AMini*UM).AND.Km>1)AMPR=AMPR-(AMini*UM)
      PL=RHO(Km)
      Tp=Ty(Km)
C- CHEM=CHEM0(Km);
      DO09954ii=1,Natom
...
09954 CONTINUE
      ...
      If(AMPR>0.99D0*(AMOUT*UM).AND.Km>Nzon/3)then
...
09951 CONTINUE
      AMPR=AMPR*UM
      WRITE(4,193)Km,-AMPR,Ry(Km),Uy(Km)*1.D+6/(UTIME*CRAP),Ty(Km),TpRAD
     *,PLLOG,PLOG,QVLOG,QRTLG,WRK(Km,1),ENG,Flum,WRKX(Km),Km,barionn,Xe*
     *barionn,
C- barion n cm^{-3}, n_e
...
C*
      ...
      WRITE(4,93)Km,AMPR,Ry(Km),Uy(Km)*1.D+6/(UTIME*CRAP),
...
C- t in days
     *
C- tout*Utime/8.64d4,
     *Km,log10(AMPR),log10(UR*Ry(Km)),Uy(Km)*1.D+6/(UTIME*CRAP),log10(ma
     *x(UTP*Ty(Km),1.d0)),log10(max(UTP*TpRAD,1.d0)),PLLOG,PLOG,QVLOG,lo
     *g10(max(eng,1.d-50)),Flum*1.d-40,WRKX(Km),barionn,Xe*barionn,(YABU
     *N(ii,Km)*AZ(ii),ii=1,Natom)
C- n_bar, n_e (cm^{-3}), composition (15 mass fractions X_k=Y_k*A_k)
      else
      WRITE(81,281)0,Km,log10(AMPR),log10(UR*Ry(Km)),Uy(Km)*1.D+6/(UTIME
     **CRAP),log10(max(UTP*Ty(Km),1.d0)),log10(max(UTP*TpRAD,1.d0)),PLLO
     *G,PLOG,QVLOG,log10(max(eng,1.d-50)),Flum*1.d-40,WRKX(Km),barionn,X
     *e*barionn,(YABUN(ii,Km)*AZ(ii),ii=1,Natom)
C- n_bar, n_e (cm^{-3}), composition (15 mass fractions X_k=Y_k*A_k)
      endif
C- 181:FORMAT(1X,F10.2,I5,F12.6,F14.7,F9.4,6F9.3,1P,2E11.3); -- for secs
181   FORMAT(1X,F10.6,I5,F12.6,F14.7,F9.4,6F9.3,1P,4E11.3,15E15.6)
C- for days, +n_bar,n_e,+composition
281   FORMAT(10X,i1,I5,F12.6,F14.7,F9.4,6F9.3,1P,4E11.3,15E15.6)
C- +n_bar,n_e,+composition
C- 182:FORMAT(1X,F10.6,I5,16E15.6) -- time, zone, 16 elements (composition)
182   FORMAT(10X,i1,I5,16E15.6)
C- 282:FORMAT(10X,i1,I5,16E15.6)
     *
      endif
      If(fullgp)Then
...
09966 CONTINUE
      ...
      KENTR=0
      EKIN=EKIN/2.D0
      EVIR=3.D0*EVIR+Prce*Ry(1)**3-(Pcnd2+RadC3*UPI*Tpcnd2**4)*RNCND**3+
     *Pcnd2*RNcnd2**3
...
09942 CONTINUE
      ...
      WRITE(4,'('' EFFECTIVE TEMPERATURE(KELVINS) & LOG10 ='',1P,2E15.5)
     *')TEFF,DLOG10(TEFF)
      WRITE(4,'('' VOLUME GAINS POWER (1E+50 ERG/S) ='',1P,E15.5)')ELVOL
     **(UEPRI/(UTIME*CRAP))
      WRITE(4,'('' SURFACE LUMINOSITY (1E+50 ERG/S) ='',1P,E15.5)')ELSUR
     *F*(UEPRI/(UTIME*CRAP))
      WRITE(4,'('' TOTAL  GAINS POWER (1E+50 ERG/S) ='',1P,E15.5)')ELTOT
     **(UEPRI/(UTIME*CRAP))
      WRITE(4,'('' RADIAT.  ENERGY  (1E+50 ERG) ='',1P,E15.5,10X,       
     *     '' TOTAL    ENERGY  (1E+50 ERG) ='',E15.5)')ERADD*UEPRI,ETOT*
     *UEPRI
      WRITE(4,'('' KINETIC  ENERGY  (1E+50 ERG) ='',1P,E15.5,10X,       
     *     '' GAINED   ENERGY  (1E+50 ERG) ='',E15.5)')EKIN*UEPRI,ELOST*
     *UEPRI
      WRITE(4,'('' GRAVIT.  ENERGY  (1E+50 ERG) ='',1P,E15.5,10X,       
     *     '' VISCOUS  VIRIAL  (1E+50 ERG) ='',E15.5)')EGRAV*UEPRI,VISVI
     *R*UEPRI
      WRITE(4,'('' VIRIAL   ENERGY  (1E+50 ERG) ='',1P,E15.5,10X,       
     *     '' VIRIAL  BALANCE  (1E+50 ERG) ='',E15.5)')EVIR*UEPRI,BALVIR
     **UEPRI
      WRITE(4,'('' THERMAL  ENERGY  (1E+50 ERG) ='',1P,E15.5,10X,       
     *     '' TOTAL   BALANCE  (1E+50 ERG) ='',E15.5)')ETERM*UEPRI,BALEN
     *G*UEPRI
     ...
     *,BALENG*UEPRI/(ETOT*UEPRI)
      ...
      WRITE(4,'(''   OB.T(D)         TEFF(10**3) RPH         VPH'',     
     *       ''         Xph     HUSED     MBOL      -U-       -B-'',    
     *        ''       -V-       -U-B-     -B-V-'')')
      WRITE(4,'(1X,1P,G12.5,'' E'',0P,F9.3,3F12.3,1P,E10.2,0P,6F10.3)')(
     *tout-tretard)*(UTIME*CRAP)/8.64D+04,TEFF*1.D-03,RPH,VPH*1.D+06/(UT
     *IME*CRAP),XPH,HUSED*UTIME*CRAP,MBOL,MU,MB,MV,MU-MB,MB-MV
C-  CALL PRIMAG;
C- WRITE(@Wres,*)' fullgp=',fullgp,'   Ipout=',Ipout;
      IF(.NOT.(fullgp))GOTO09936
C: draw graphic *
C- WRITE(@Wres,'(''%GR:'')');
...
C-                       wH(Lfr)*log(max(Y(@L),1.d-100)));
      if(Lfr0<Nfrus)then
      Hobsg=cH(Lfr)*exp((1.d0-wH(Lfr))*log(max(Yout(NVARS*NZON+(NZON-NCN
     *D)*(L+1)-1+KRAD),1.d-100))+wH(Lfr)*log(max(Yout(NVARS*NZON+(NZON-N
     *CND)*L-1+KRAD),1.d-100)))
      else
...
C- obs.lum.
      SUM=0.
      DO09930L=1,NFRUS+dLfrMax
      ttt(L)=SNGL(LOG10(MAX(ABS(ttt(L)*freqob(L)**3),1.d-100)))
      ttt(L)=ttt(L)+CLUMNU
C- erg/(sec*Hz)
      SUM=SUM+10.**ttt(L)*(freq(L+1)-freq(L))*(BoltzK*UTp/(2.d0*pi*Hplan
     *c))
09930 CONTINUE
      WRITE(4,*)' Rest observer Lum=',Obflum,SUM
C-    pause;
      WRITE(4,*)'   L        lg nu         comov        rest',' observer
     * lg(L_nu)  erg/(s Hz)'
      DO09927L=1,Nfrus
      write(4,'(i4,3f14.3)')L,log10(freqmn(L)*UFREQ),
C-               SNGL(LOG10(MAX(ABS(Y(@L)*RY(NZON-1)**2*FREQMN(L)**3),
     *SNGL(LOG10(MAX(ABS(Yout(NVARS*NZON+(NZON-NCND)*L-1+KRAD)*RY(NZON-1
     *)**2*FREQMN(L)**3),1.d-100)))+Clumnu,ttt(L)
09927 CONTINUE
C:  PRINT FJ & FH *
...
C- FJ
      If(NFRUS>10)WRITE(4,'(7X,10F9.3)')(log10(max(CJnu*freqmn(L)**3
C-           *Y(NVARS*NZON+(K-NCND)+(NZON-NCND)*(L-1),1),1.d-299)),
...
C- HEADER FOR FREQ
      ...
      DO09921K=NCND+1,NZON
      WRITE(4,'(1X,I4,2X,10F9.3)')K,(log10(max(CJnu*freqmn(L)**3*Yout(NV
     *ARS*NZON+KRAD+(K-NCND)+(NZON-NCND)*(L-1)),1.d-299)),L=1,Lfin)
C- FH
      If(NFRUS>10)WRITE(4,'(7X,10F9.3)')(log10(max(CJnu*freqmn(L)**3*You
     *t(NVARS*NZON+KRAD+(K-NCND)+(NZON-NCND)*(L-1)),1.d-299)),L=11,NFRUS
     *)
C- FH
09921 CONTINUE
C
...
46    FORMAT(7X,1P,10(E9.2,L2))
      Lbeg=1
      Lfin=MIN(NFRUS,10)
09918 IF(.NOT.(Lbeg<Nfrus))GOTO09917
      WRITE(4,'(//)')
...
C- HEADER FOR FREQ
      ...
      DO09915ip=0,NP
C- rays
      WRITE(4,'(1X,1p,e9.3,2X,0p,10F9.3)')pray(ip)*UR,(log10(max(CJnu*fr
     *eqmn(L)**3*fout(ip,L),1.d-299)),L=Lbeg,Lfin)
09915 CONTINUE
      Lbeg=Lfin+1
...
09917 CONTINUE
C:  plots  for spectra *
      DO09912K=NCND+1,NZON
C- RADIATION
      If(NZON-NCND>5)Then
      IF(.NOT.(K==NCND+1))GOTO09909
      KK=2
      GOTO09903
09909 CONTINUE
      IF(.NOT.(K==NCND+(NZON-NCND)/4))GOTO09908
      KK=3
      GOTO09903
09908 CONTINUE
      IF(.NOT.(K==NCND+(NZON-NCND)/2))GOTO09907
      KK=4
      GOTO09903
09907 CONTINUE
      IF(.NOT.(K==NCND+3*(NZON-NCND)/4))GOTO09906
      KK=5
      GOTO09903
09906 CONTINUE
      IF(.NOT.(K==NZON-1))GOTO09905
      KK=6
      GOTO09903
09905 CONTINUE
      IF(.NOT.(K==42.or.K==43))GOTO09904
      kk=7
      GOTO09903
09904 CONTINUE
      GOTO09912
...
C- WORKX(L)=FLOAT(L);
09902 CONTINUE
      WRITE(4,'(A,I3,A,1P,E12.3,A,(1P,8E12.3))')' K=',K,'   tau=',tau(K)
     *,' FJ(L,K)=',(FJ(ill),ill=1,NFRUS)
09912 CONTINUE
CBAR*  use  BARKUK *
C: tau in all frequencies for selected zone *
      Nztau=43
      write(4,*)' L / lambda / tau in zon / tauabs / hapabs / hapsum /',
     *' hapscat / cooling / heating ',' for Km = ',Nztau
      DO09899LL=1,Nfrus
      tauzon(LL)=0.d0
09899 CONTINUE
      DO09896Km=Nzon,Nztau,-1
      PL=RHO(Km)
      Tp=Ty(Km)
C- CHEM=CHEM0(Km);
      DO09893ii=1,Natom
...
09893 CONTINUE
      ...
      if(Km==Nztau)then
      write(4,'(1x,i4,1p,2e13.4,6e11.3)')ll,Cs*1.d8/(freqmn(ll)*Ufreq),(
     *Ry(Km)-Ry(Km-1))*HAPPAL(LL)*PL,(Ry(Km)-Ry(Km-1))*HAPAbs(LL)*PL,log
     *10(max(1.d-50,hapabs(ll)/(ur*urho))),log10(max(1.d-50,happal(ll)/(
     *ur*urho))),log10(max(1.d-50,(happal(ll)-hapabs(ll))/(ur*urho))),HA
     *PAbs(LL)*black(LL,Tp)*weight(LL),HAPAbs(LL)*Y(NVARS*NZON+Nztau-NCN
     *D+(NZON-NCND)*(LL-1),1)*weight(LL)
      endif
09890 CONTINUE
09896 CONTINUE
      write(4,*)' L / lambda / tau for Km = ',Nztau
      write(4,'(1x,i5,2f15.5)')(ll,Cs*1.d8/(freqmn(ll)*Ufreq),tauzon(ll)
     *,ll=1,NFRUS)
09936 CONTINUE
      RETURN
...
93    FORMAT(1X,I3,F9.5,F12.7,F8.4,F10.3,F8.3,4F7.2,1P,4E10.2,I5,11e10.2
     *)
193   FORMAT(1X,I3,1P,E9.2,0P,F12.7,F8.4,F10.3,F8.3,4F7.2,1P,4E10.2,I5,1
     *1e10.2)
94    FORMAT(' ZON',3X,'AM/SOL',6X,'R',F3.0,6X,'V',F3.0,5X,'T',F3.0,4X,'
     *Trad5 lgD',F3.0,' lgP',F3.0,
...
95    FORMAT(' ZON',5X,'AM/SOL',6X,'R',F3.0,10X,'V',F3.0,10X,'T',F3.0,5X
     *,'LG D',F3.0,' LG P',F3.0,' NREG   ENGNUC    XCARB     UCONV     E
     *NTROP   FLCONV   ZON')
      END
...
C- new!
     ...
     *zon/(Mzon/2)+1,2),hptabsc(Nfreq,14,14,Mzon/(Mzon/2)+1,2),
...
C: FIND PARAMETERS OF PHOTOSPHERE *
      ...
      IRph=0
      kmrep=0
09881 IF(.NOT.((I>1.AND.(tau(I)<0.64D0.or.tauTp<0.64D0)).or.I>Nzon-5))GO
     *TO09880
      PL=rhop(I)
      Tp=Typ(I)
C-       write(*,'(a,i5,1p,2e12.3)')' in primag I rhop Typ:',I,rhop(I),Typ(I);
      ...
      I=I-1
      If(tau(I)>=0.1d0.AND.kmRep==0)kmRep=max(I,kmnick)
      If(tau(I)>=0.64D0.AND.IRph==0)IRph=I
...
C-       write(*,'(a,i5,1p,e12.4)')' primag L Hedd:', L,Hedd(L);
      SUMH=SUMH+Y(NVARS*NZON+(NZON-NCND)*L-1+KRAD,1)*WEIGHT(L)/(HEDD(L)*
     *(1.D0-HEDD(L)))
09878 CONTINUE
...
C-  Endif;
      I=1
09875 IF(.NOT.(I<NZON-1.AND.Ryp(I+1)<RPH))GOTO09874
      I=I+1
      GOTO09875
09874 CONTINUE
C- Ryp(I+1)>=RPH
      VPH=uyp(I+1)+(uyp(I)-uyp(I+1))/(Ryp(I)-Ryp(I+1))*(RPH-Ryp(I+1))
      XPH=YABUN(1,I+1)+(YABUN(1,I)-YABUN(1,I+1))/(Ryp(I)-Ryp(I+1))*(RPH-
     *Ryp(I+1))
      If(MOD(NSTEP,MAX0(1,NOUT/10))==0)write(*,*)' primag Tph Rph Iph=',
     *I,Tph,Rph
C%MG:
C  _DO I=1,NZON;
C    WRKX(I)=FLOAT(I);
...
C- FOR U
C-   print *, L,Y(@L),Y(@L1);
      If(L<NFRUS.AND.abs(Y(NVARS*NZON+(NZON-NCND)*(L+1)-1+KRAD,1))>1.d-1
     *00)Then
      FHU=EXP(LOG(ABS(Y(NVARS*NZON+(NZON-NCND)*(L+1)-1+KRAD,1)))+LOG(ABS
     *(Y(NVARS*NZON+(NZON-NCND)*L-1+KRAD,1)/Y(NVARS*NZON+(NZON-NCND)*(L+
     *1)-1+KRAD,1)))*DLOGNU(L)*LOG((CCL/3.65D+03)/FREQMN(L+1)))
...
C-   print *, L, Y(@L),Y(@L1);
      If(L<NFRUS.AND.abs(Y(NVARS*NZON+(NZON-NCND)*(L+1)-1+KRAD,1))>1.d-1
     *00)Then
      FHB=EXP(LOG(ABS(Y(NVARS*NZON+(NZON-NCND)*(L+1)-1+KRAD,1)))+LOG(ABS
     *(Y(NVARS*NZON+(NZON-NCND)*L-1+KRAD,1)/Y(NVARS*NZON+(NZON-NCND)*(L+
     *1)-1+KRAD,1)))*DLOGNU(L)*LOG((CCL/4.4D+03)/FREQMN(L+1)))
C-           LOG(ABS(Y(@L)/Y(@L1)))/(FREQMN(L)-FREQMN(L+1))
C-       *(@B-FREQMN(L+1)));
      else
...
C-   print *, L,Y(@L),Y(@L1);
      If(L<NFRUS.AND.abs(Y(NVARS*NZON+(NZON-NCND)*(L+1)-1+KRAD,1))>1.d-1
     *00)Then
      FHV=EXP(LOG(max(1.d-100,ABS(Y(NVARS*NZON+(NZON-NCND)*(L+1)-1+KRAD,
     *1))))+LOG(max(1.d-100,ABS(Y(NVARS*NZON+(NZON-NCND)*L-1+KRAD,1)/Y(N
     *VARS*NZON+(NZON-NCND)*(L+1)-1+KRAD,1))))*DLOGNU(L)*LOG((CCL/5.5D+0
     *3)/FREQMN(L+1)))
C-           LOG(ABS(Y(@L)/Y(@L1)))/(FREQMN(L)-FREQMN(L+1))
C-       *(@V-FREQMN(L+1)));
      else
...
C- CFLUX=60/PI**4 * SIGMA * UTp**4
      MU=-2.5D0*(LOG10(max(CFLUX*FHU*(CCL/3.65D+03)**5/CCL*(Ryp(NZON-1)*
     *UR/3.0857D19)**2,1.d-10))+8.37D0)
      MB=-2.5D0*(LOG10(max(CFLUX*FHB*(CCL/4.4D+03)**5/CCL*(Ryp(NZON-1)*U
     *R/3.0857D19)**2,1.d-10))+8.18D0)
      MV=-2.5D0*(LOG10(max(CFLUX*FHV*(CCL/5.5D+03)**5/CCL*(Ryp(NZON-1)*U
     *R/3.0857D19)**2,1.d-10))+8.42D0)
CG* PREPARE WRK,WRKX & CALL BARKUK TO CONTROL GAS DYNAMICS *
      ObsLum=0.d0
      DO09872L=1,NFRUS
      Obslum=Obslum+Y(NVARS*NZON+(NZON-NCND)*L-1+KRAD,1)*WEIGHT(L)*(1.d0
     *+(uyp(Nzon-1)/clight)*(1.d0+EddJ(Nzon-1,L))/HEdd(L))
C- observer
...
C-        endif;
      if(Lfr0<Nfrus)then
      Hobsg=cH(Lfr)*exp((1.d0-wH(Lfr))*log(max(Y(NVARS*NZON+(NZON-NCND)*
     *(L+1)-1+KRAD,1),1.d-100))+wH(Lfr)*log(max(Y(NVARS*NZON+(NZON-NCND)
     **L-1+KRAD,1),1.d-100)))
      else
...
C- obs.lum. according Flsave
      ...
      write(*,'(a,1p,4e12.3)')'  lbalsw: Obslum Obflum CLUMF Hnorm: ',Ob
     *slum,Obflum,CLUMF,Hnorm
C-       pause;
      endif
...
C- save Rph from tau=0.64
      ...
      WRITE(4,'(1X,1P,G12.5,0P,F11.3,3F12.3,1P,E10.2,0P,6F10.3)')Time,TE
     *FF*1.D-03,RPH,VPH*1.D+06/(UTIME*CRAP),XPH,HUSED*UTIME*CRAP,MBOL,MU
     *,MB,MV,MU-MB,MB-MV
      endif
