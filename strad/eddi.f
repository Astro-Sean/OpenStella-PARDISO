     ...
     *d0-(exp(-(FREQMN(Lbl)/Tpbl))))**2
      ...
      write(*,*)' Entered Feau - Eddi at Nstep=',Nstep,' Nfrus=',Nfrus
      Ip=0
      kzer=0
      nrdial=0
      tretL=.false.
      ...
      NPE=max(NPE,2)
      NPC=15+15-NPE
      write(*,'(a,4i6,1p,g12.3)')' Npe  Npc new/old  Ncond, Rcore:',NPE,
     *NPC,incnd,Kntold,Rcore
      Pray(0)=0.D0
      Pray(NP+1)=ROUT
...
09970 CONTINUE
      ...
      kmhap=Knth+1
      CALLOPACIT
      FLCOR1=((Ry(Knth)*Tp1)**4-(Ry(Knth)*Tp)**4)*Tp1**4/(CAP1*(DM(Knth)
     *+DM(Knth+1))*(Tp1**4+Tp**4))
      FLCOR2=((Ry(Knth)*Tp1)**4-(Ry(Knth)*Tp)**4)*Tp**4/((CAPPA*(DM(Knth
     *)+DM(Knth+1))+0.d0*Ry(Knth)**2)*(Tp1**4+Tp**4))
      FL1=FLCOR1+FLCOR2
      FDLEFT(Lfr)=FL1*blacKD(Lfr,(0.5D0*(Tp+Tp1)))*CAPPA/(1.d0*(0.5D0*(T
     *p+Tp1))**4*Rleft**2*HAPW(Knth+1,Lfr))
      endif
      KLASTS=Nzon
      DO09967Ip=NP,0,-1
      Kzer=Nzon
09964 IF(.NOT.(Pray(Ip)<RM(Kzer).AND.Kzer>Knth))GOTO09963
      Kzer=Kzer-1
      GOTO09964
09963 CONTINUE
...
09955 CONTINUE
      ...
      chiex=HAPW(Kpl,Lfr)
      dtau(Kpl,Ip)=(Amu(Ip,Kpl)*RM(Kpl)-Amu(Ip,Kpl-1)*RM(Kpl-1))*chiex*r
     *ho(Kpl)
      If(Scat.AND.Nstep>1.AND.Kpl>Kntold)then
      fdirsumm=(chiab*fblack(fr0,Ty(Kpl))+(chiex-chiab)*FJr(Kpl,Lfr))/Ha
     *pW(Kpl,Lfr)*(1.d0-exp(-dtau(Kpl,Ip)/2.d0))
      else
      fdirsumm=fblack(fr0,Ty(Kpl))*(1.d0-exp(-dtau(Kpl,Ip)/2.d0))
      endif
      tauz=dtau(Kpl,Ip)/2.d0
      K=Kpl-1
09946 IF(.NOT.(K>Kz(Ip)))GOTO09945
      ...
      If(Scat.AND.Nstep>1.AND.K>Kntold)then
      fdirsumm=fdirsumm+(chiab*fblack(fr0,Ty(K))+(chiex-chiab)*(FJr(K,Lf
     *r0)*(1.d0-wfr0)+FJr(K,min(Nfrus,Lfr0+1))*wfr0))/chiex*exp(-tauz)*(
     *1.d0-exp(-dtau(K,Ip)))
      else
      fdirsumm=fdirsumm+fblack(fr0,Ty(K))*exp(-tauz)*(1.d0-exp(-dtau(K,I
     *p)))
      endif
      tauz=tauz+dtau(K,Ip)
      K=K-1
      GOTO09946
09945 CONTINUE
      if(K==Kz(ip).AND.tauz<1.d+02)then
09943 IF(.NOT.(tauz<1.d+02.AND.K<Nzon))GOTO09942
      K=K+1
      duz=Amu(Ip,Kpl)*umr(Kpl)+Amu(Ip,K)*umr(K)
      fr0=freqob(Lfr)*(1.d0+duz/clight)
      fr0log=log(fr0)
...
09952 CONTINUE
     ...
     *)
      If(Scat.AND.Nstep>1.AND.Ki>Kntold)then
      fdirsumm=(chiab*fblack(fr0,Ty(Ki))+(chiex-chiab)*FJr(Ki,Lfr))/HapW
     *(Ki,Lfr)*(1.d0-exp(-dtau(Ki,Ip)/2.d0))
      else
      fdirsumm=fblack(fr0,Ty(Ki))*(1.d0-exp(-dtau(Ki,Ip)/2.d0))
      endif
      tauz=dtau(Ki,Ip)/2.d0
      K=Ki
09934 IF(.NOT.(K<Nzon))GOTO09933
      K=K+1
...
09928 CONTINUE
      ...
      FJLfr(K)=FJLfr(K)+(FSdir(Ipnmu,K)+FSdir(Ipnmu+1,K))*(Amu(Ipnmu,K)-
     *Amu(Ipnmu+1,K))
      IF(K<Nzon)then
      if(Hapw(K,Lfr)*rho(k)*(Ry(K+1)-Ry(K))<Fittau/Tautol)then
      FRADH(K,Lfr)=FRADH(K,Lfr)+(FDdir(Ipnmu,K)+FDdir(Ipnmu+1,K))*(Amu(I
     *pnmu,K)**2-Amu(Ipnmu+1,K)**2)
      endif
      Endif
      FRADK(K)=FRADK(K)+(FSdir(Ipnmu,K)+FSdir(Ipnmu+1,K))*(Amu(Ipnmu,K)*
     **3-Amu(Ipnmu+1,K)**3)
09922 CONTINUE
...
09925 CONTINUE
      ...
      FRADH(Nzon,Lfr)=FRADH(Nzon,Lfr)+(FSdir(Ipnmu,Nzon)+FSdir(Ipnmu+1,N
     *zon))*(Amu(Ipnmu,Nzon)**2-Amu(Ipnmu+1,Nzon)**2)
09919 CONTINUE
      FRADH(Nzon,Lfr)=0.25d0*(FRADH(Nzon,Lfr)+2.d0*FSdir(NMU(Nzon),Nzon)
     **Amu(NMU(Nzon),Nzon)**2)
      RADH=0.D0
      DO09916Ipnmu=NMU(Nzon)-1,0,-1
      RADH=RADH+(FSdir(Ipnmu,Nzon)+FSdir(Ipnmu+1,Nzon))*(Amu(Ipnmu,Nzon)
     ***2-Amu(Ipnmu+1,Nzon)**2)
09916 CONTINUE
      RADH=RADH+2.d0*FSdir(NMU(Nzon),Nzon)*Amu(NMU(Nzon),Nzon)**2
      If(ABS(FJLFR(Nzon))>1.d-50)then
      HEdN(Lfr)=RADH/(4.D0*FJLfr(Nzon))
      else
      HEdN(LFR)=0.5D0
      endif
      IF(Lfr==LubvB)EDDH(Nzon)=HEdN(Lfr)
      WRITE(*,'(A,I3)')' Eddi Lfr: ',Lfr
09979 CONTINUE
      If(incnd==ncnd.and..not.tretL)then
      Lfr=20
      ...
      Lfr0=int(xnu)
      dLfrMax=Lfr-Lfr0
      write(*,*)' xnu for Lfr=20, int(xnu), Lfr0, dLfrMax:',xnu,int(xnu)
     *,Lfr0,dLfrMax
      ...
      xnu=(zerfr-fr0log)*dlognu(1)
      Lfr0=max(int(xnu),1)
      if(Lfr==1)write(*,*)' obsflux_fout: Lfr Lfr0 xnu ',Lfr,Lfr0,xnu
      if(Lfr>dLfrMax)then
      if(Lfr0<1)stop' Lfr0 must be >= 1 in feau for Lfr>dLfrMax'
      wfr0=(freqob(Lfr0+1)-fr0)/(freqob(Lfr0+1)-freqob(Lfr0))
      fout(Ipnmu,Lfr)=exp(log(max(fcom(Ipnmu,Lfr0),1.d-50))*wfr0+log(max
     *(fcom(Ipnmu,min(Nfrus,Lfr0+1)),1.d-50))*(1.d0-wfr0))
      else
      if(Lfr0>=1)then
      wfr0=(freqob(Lfr0+1)-fr0)/(freqob(Lfr0+1)-freqob(Lfr0))
      fout(Ipnmu,Lfr)=exp(log(max(fcom(Ipnmu,Lfr0),1.d-50))*wfr0+log(max
...
09907 CONTINUE
      ...
      Hobs(Lfr)=Hobs(Lfr)+(fout(Ipnmu,Lfr)+fout(Ipnmu+1,Lfr))*(abermu(Ip
     *nmu)**2-abermu(Ipnmu+1)**2)
      if(Lfr<=Nfrus)Hcomov=Hcomov+(fcom(Ipnmu,Lfr)+fcom(Ipnmu+1,Lfr))*(a
     *mu(Ipnmu,Nzon)**2-amu(Ipnmu+1,Nzon)**2)
09901 CONTINUE
      if(Lfr<=Nfrus)Hcomov=0.125d0*(Hcomov+fcom(NMU(Nzon),Lfr)*amu(NMU(N
     *zon),Nzon)**2)
      Hobs(Lfr)=0.125d0*(Hobs(Lfr)+fout(NMU(Nzon),Lfr)*abermu(NMU(Nzon))
     ***2)
09904 CONTINUE
      Hnorm=0.d0
      tlum=0.d0
      ...
      if(L<=Nfrus)tlum=tlum+fradH(Nzon,L)*WEIGHT(L)*(1.d0+(uy(Nzon)/clig
     *ht)*(1.d0+EddN(Nzon,L))/HEdn(L))
09898 CONTINUE
      ...
      wH(Lfr)=wfrlog
      If(abs(FradH(Nzon,min(Lfr0+1,Nfrus))*Hobs(Lfr)*FradH(Nzon,Lfr0))>1
     *.d-50**3.and.FradH(Nzon,min(Lfr0+1,Nfrus))>0.d0.and.FradH(Nzon,Lfr
     *0)>0.d0)then
      cH(Lfr)=Hobs(Lfr)/exp((1.d0-wH(Lfr))*log(FradH(Nzon,min(Lfr0+1,Nfr
     *us)))+wH(Lfr)*log(FradH(Nzon,Lfr0)))
      else
      cH(Lfr)=cH(max(1,Lfr-1))
      endif
      else
      if(FradH(Nzon,Lfr)>1.d-50)then
