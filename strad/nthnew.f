...
09999 CONTINUE
      ...
      TauMIN=0.D0
      tauste=0.d0
09996 IF(.NOT.(Kmth>1.AND.(Kmth>=NTM.OR.TauMIN<=Tautol*ThtTau.OR.Kmth>=I
     *NCND)))GOTO09995
      TP=Ty(Kmth)
...
09993 CONTINUE
      ...
      If(L==1.OR.TauT(L)<TauMIN)TauMIN=TauT(L)
      If(Kmth==Nthick(L))TauL(L)=TauT(L)
      If(TauT(L)>ThtTau.AND.NthNEW(L)==0)NthNEW(L)=Kmth
...
09995 CONTINUE
      DO09987L=1,NFRUS
      If((Taul(L)>Tautol*ThtTau.OR.Taul(L)*Tautol<ThtTau.OR.MthNEW(L)<Nt
     *hick(L)).AND.MIN(MthNEW(L),NthNEW(L),NZON-3)/=Nthick(L))Then
...
09987 CONTINUE
      ...
      If(LTHICK(L,KM))Then
      If(abs(Y(NVARS*NZON-NCND+(NZON-NCND)*(L-1)+KM,1)/BLA-1.D0)>EPTHIN.
     *or.KM>NTHICK(L))Then
      LTHICK(L,KM)=.FALSE.
      Newth=.TRUE.
      Endif
      Else
      If(abs(Y(NVARS*NZON-NCND+(NZON-NCND)*(L-1)+KM,1)/BLA-1.D0)<EPTHIC.
     *AND.KM<=NTHICK(L))Then
      LTHICK(L,KM)=.TRUE.
      Newth=.TRUE.
      Endif
      Endif
...
09975 CONTINUE
      ...
      If(ISM/=0)WRITE(4,*)'Smooth done ',ISM,' times'
      return
      end
