...
C- for BRAYTON, GUSTAVSON & HACHTEL method
      SUBROUTINEVTSTIF
      USE pardiso_wrap_mod, ONLY: pardiso_solve
...
C: COMMON & VARIABLES FOR LINEAR EQS SOLVER Y12m *
      Parameter(MAXIT=15)
      LOGICALLBLOCK,GROW,ABORT(4)
      COMMON/JSAVE/ASAVE(NZ),IRS(NZ),ICS(NZ)
      real*8DB(NYDIM),XSAVE(NYDIM)
      real*8a1(NZ)
C-!    Dimension B(NYDIM), b1(NYDIM),  Bsave(NYDIM), X(NYDIM), Y(NYDIM);
      real*8b1(NYDIM),Bsave(NYDIM),X(NYDIM),ysave(NYDIM)
      integersn(NZ)
      parameter(iha=NYDIM)
C-!       parameter (nonz_max=15) -- now NZ
C-!       parameter (nn=3*nonz_max)
      parameter(nn=3*NZ)
C-!       parameter (nn1=3*nonz_max)
      parameter(nn1=3*NZ)
      real*8aflag(11)
      real*8pivotArr(iha)
      integerha(iha,13)
...
C: DATA FOR SPARSE MATRIX SubroutineS Y12m* *
      DATALBLOCK/.True./,GROW/.TRUE./,ABORT/.FALSE.,.TRUE.,.FALSE.,.TRUE
     *./,PIVOT/1.D-01/
...
C------ '-->Entering Node %G:'
09990 IF(.NOT.(RETCOD==0))GOTO09989
...
09969 IF(.NOT.(.NOT.CORRCO.AND.ITER<MAXITE))GOTO09968
...
C- if(Nstep>1586 & EVALJA)then;
      if(Nstep>=NDebug.AND.EVALJA)then
...
C- radius
      ...
      elseIf(Nzon<JTST.AND.JTST<2*Nzon)then
C- velocity
      If(JTST==Nzon+1)then
...
C------ '-->Entering Node %G_COR_DEB_COMPR:'
      FOUNDI=.FALSE.
      IL=1
09936 IF(.NOT.(IL<=ILAST.AND..NOT.FOUNDI))GOTO09935
...
09935 CONTINUE
      IF(FOUNDI)THEN
      If(KTST/=JTST)THEN
      If(ABS(AJAC(IRN(NZ+IL))*DeltY+HL*DNDVAR(KTST))>1.D-1*ABS(AJAC(IRN(
     *NZ+IL))*DeltY).AND.ABS(AJAC(IRN(NZ+IL))/HL)>1.D-4)Then
      If(abs(DeltY)>DFLTZR*abs(DNDVAR(KTST)))then
      WRITE(4,'(/A/2I7,1P,3G12.3)')'  K,J,AJAC,Stella df/dY,Num DN(K)/DV
     *(J):   ',KTST,JTST,AJAC(IRN(NZ+IL)),-AJAC(IRN(NZ+IL))/HL,DNDVAR(KT
     *ST)/DeltY
      else
      WRITE(4,'(/A/2I7,1P,3G12.3)')'  K,J,AJAC,(df/dY,DN/DV)*DeltaDeb:  
     *',KTST,JTST,AJAC(IRN(NZ+IL)),-AJAC(IRN(NZ+IL))*DeltY/HL,DNDVAR(KTS
     *T)
      endif
...
C------ '-->Entering Node %G_COR_DEB_COMPR_PhysInfo:'
      	...
      	WRITE(4,'(3(A,1P,E12.4))')	'  Y(K)=',Y(KTST,1),'    Fsave(K)=',Fs
     *ave(KTST),	'    Ydot(K)=',FSAVE(NYDIM+KTST)
      WRITE(4,'(2(A,I5),A,1P,E12.4)')' Control IR=',KTST,	'  IC=',JTST,'
     *   AJAC=',AJAC(I)
      endif
      	enddo
C- this is for control of found AJAC;
      ...
      Ivar=(ICW-1)/NZON+1
      WRITE(4,'(3A,I3)')' Derivative over ',CharVarName(Ivar)(1:len_trim
     *(CharVarName(Ivar))),' in  zon=',Izon
      else
      ICW=ICW-NZON*NVARS
      If(ICW<=KRAD)Then
      L=(ICW-1)/(NZON-NCND)+1
      ...
      Ivar=(KTST-1)/NZON+1
      WRITE(4,'(3A,I3)')' for Ydot of ',CharVarName(Ivar)(1:len_trim(Cha
     *rVarName(Ivar))),' in  zon=',Izon
      else
...
C------ '<--Leaving  Node %G_COR_DEB_COMPR_PhysInfo:'
      ...
      If(ABS((AJAC(IRN(NZ+IL))-1.D0)*DeltY+HL*DNDVAR(KTST))>1.D-1*ABS((A
     *JAC(IRN(NZ+IL))-1.D0)*DeltY).AND.ABS((AJAC(IRN(NZ+IL))-1.D0)/HL)>1
     *.D-4)Then
      If(abs(DeltY)>DFLTZR*abs(DNDVAR(KTST)))then
      WRITE(4,'(/A/2I7,1P,3G12.3)')'  K,J,AJAC,Stella df/dY,Num DN(K)/DV
     *(J):  ',KTST,JTST,AJAC(IRN(NZ+IL)),(1.D0-AJAC(IRN(NZ+IL)))/HL,DNDV
     *AR(KTST)/DeltY
      else
      WRITE(4,'(/A/2I7,1P,3G12.3)')'  K,J,AJAC,(df/dY,DN/DV)*DeltaDeb:  
     *',KTST,JTST,AJAC(IRN(NZ+IL)),(1.D0-AJAC(IRN(NZ+IL)))*DeltY/HL,DNDV
     *AR(KTST)
      endif
C: give info on variable number KTST *
C------ '-->Entering Node %G_COR_DEB_COMPR_PhysInfo:'
...
C------ '<--Leaving  Node %G_COR_DEB_COMPR_PhysInfo:'
      ...
      ELSE
      IF(ABS(DNDVAR(KTST))>1.D-4)then
      If(abs(DeltY)>DFLTZR*abs(DNDVAR(KTST)))then
      WRITE(4,'(/A,2I7,1P,2G12.3)')' NO JAC: K,J,DN(K)/DV(J)  ',KTST,JTS
     *T,DNDVAR(KTST)/DeltY
      else
      WRITE(4,'(/A,2I7,1P,2G12.3)')' NO JAC: K,J,DN(K)/DV(J)*DeltaDeb ',
     *KTST,JTST,DNDVAR(KTST)
      endif
      endif
      ENDIF
...
C         NUMSING=.true.;
C         NEEDBR=.true.;
C       ITER=MAXITE; EVALJA=.FALSE.; OLDJAC=.FALSE.;
C       Ifail=33;
C       -- Variables are given the values such as to guarantee that
C       -- control passes to the node %GY - to try a new smaller step
C      else;
...
C------ '-->Entering Node %G_Cor.L_NUMSING:'
      WRITE(*,'(3(A,1P,E12.4))')'Y(IR)=',Y(IRW,1),'    Fsave(IR)=',Fsave
     *(IRW),'    Ydot(IR)=',FSAVE(NYDIM+IRW)
      DO09924I=1,NZMOD
      IF(IRS(I)==IRW)THEN
      WRITE(*,'(2(A,I5),A,1P,E12.4)')'IR=',IRS(I),'  IC=',ICS(I),'   AJA
     *C=',ASAVE(I)
      ICW=ICS(I)
      If(ICW<=NZON*NVARS)then
      Izon=MOD(ICW-1,NZON)+1
...
09918 CONTINUE
      ...
      iflag(i)=0
      enddo
C-!  IFLAG(11)  .ge.  2  should be assigned, it is the number of iterations.
      iflag(11)=MAXIT
C-!       aflag(1) -- has the same meaning as old 1/pivot == tolerance for the choice of pivot
C
C      write(*,'(a)')'i,ajac(i),irn(i),icn(i)';
C      do i=1,min(5,nzmod);
C        write(*,'(i10,1pg12.3,0p,2i10)')i,ajac(i),irn(i),icn(i);
C      enddo;
C      If(nzmod>4)then;
C        do i=nzmod-4,nzmod;
C          write(*,'(i10,1pg12.3,0p,2i10)')i,ajac(i),irn(i),icn(i);
C        enddo;
C      endif;
C      *
      call pardiso_solve(N,ajac,irn,icn,nzmod,FSAVE(NYDIM+1)
     *,XSAVE,ifail)
C- RHS in FSAVE(NYDIM+1), solution in XSAVE
      write(*,*)'PARDISO error flag IFAIL=',ifail
C
C*
C   _DO I=1,N;
C      FSAVE(I+NYDIM)=FSAVE(I+NYDIM)*H-Y(I,2)-ERROR(I);
C                    -- R-H SIDE IN FSAVE(NYDIM+1:NYDIM+N)
C      DB(I)=FSAVE(I+NYDIM); -- TO SAVE R-H SIDE
C   _OD;
C
C-- ITERATIVE REFINEMENT: -- now done in y12mff
C    ITQ = 0;
C If(Ifail^=33)Then;
C   _Repeat
C        CALL M28CYN(N,AJAC,LICN,ICN,IKEEP,FSAVE(NYDIM+1),
C                    WJAC,IW,1,IDISP,RESID,Ifail);
C        -- SOLUTION IN FSAVE(NYDIM+1:NYDIM+N)
C    If(Ifail^=33)Then;
C        ERROLD=ERREST; ERREST=0.D0; XMOD=0.D0;
C      _DO ID=1,N;
C        XSAVE(ID)=XSAVE(ID)+FSAVE(ID+NYDIM);
C        XMOD=XMOD+ABS(XSAVE(ID));  -- NORMA
C        ERREST=ERREST+ABS(FSAVE(ID+NYDIM));
C        FSAVE(ID)=0.D0;
C      _OD;
C      _DO K=1,NZMOD;
C        FSAVE(IRS(K))=FSAVE(IRS(K))         -- Polish strategy
C                         +XSAVE(ICS(K))*ASAVE(K);
C      _OD;
C      _DO IL=1,N;
C        FSAVE(IL+NYDIM)=DB(IL)-FSAVE(IL);
C      _OD;
...
C           IRN(Isw)=IRS(Isw);
C           ICN(Isw)=ICS(Isw);
C           AJAC(Isw)=ASAVE(Isw);
C        _OD;
C        NEEDBR=.TRUE.; EVALJA=.TRUE.;
C        @wterm ' G_Cor.UK: ifail 33';
C   Endif;
C-- _Until ERREST<=ANOISE*XMOD ! ITQ > MAXIT;
C   _Until ERREST<=ANOISE*XMOD .or. Ifail==33
C            .or. (ITQ>2 & ERREST>ERROLD) .or. ITQ > MAXIT;
C      IF(ERREST > XMOD*.1d0*EPS) NEEDBR=.TRUE.; -- Esaulov correction
C--  IF(XMOD>EPS**3)WRITE(NCHANL,*)' RELEST=',ERREST/XMOD;
C Endif;
C*
      IF(IFAIL==14.or.IFAIL==17.or.IFAIL==18)then
      write(*,*)' matrix is STRUCTURALLY singular, check Jacobian'
      stop8
      ENDIF
C-!       IRW=Idisp(11); -- change
...
C------ '-->Entering Node %G_Cor.UV:'
      D=0.D0
C-!     write(*,'(a)')'FSAVE(I),ERROR(I),XSAVE(I)';
      DO09915I=1,N
      ERROR(I)=ERROR(I)+XSAVE(I)
      D=D+(XSAVE(I)/YMAX(I))**2
      FSAVE(I)=Y(I,1)+EL(1)*ERROR(I)
C-!       if(I<=3)write(*,'(1p6g12.3)')FSAVE(I),ERROR(I),XSAVE(I);
09915 CONTINUE
...
09876 CONTINUE
      RMAX=RMXFAI
      IF(.NOT.(ABS(H)<=(HMIN*1.00001D0)))GOTO09867
      RETCOD=1
      GOTO09866
09867 CONTINUE
      IF(.NOT.(KFLAG<=-MAXFAI))GOTO09864
...
09852 CONTINUE
      ...
      RETCOD=2
      ELSE
      write(*,'(a,i7,2i3,1p,2e12.3)')' GY: maxer, NQ, kflag, H, Hu:',max
     *er,NQ,kflag,H,Hused
      RH=RHCORR
      if(ifail==33.and.Kbad>0)then
      write(*,*)' GY: ifail 33'
C- NQ=1; LNQ=2; -- if we want 1st order
      write(*,*)' Initial from Y  R0=',Y(Kbad-1,1),'  u0=',Y(Nzon+Kbad-1
     *,1)
      write(*,*)' Initial from Y  R1=',Y(Kbad,1),'  u1=',Y(Nzon+Kbad,1)
      write(*,*)' Initial dr=',Y(Kbad,1)-Y(Kbad-1,1),'  predicted dr=',Y
     *(Kbad,1)-Y(Kbad-1,1)+(Y(Nzon+Kbad,1)-Y(Nzon+Kbad-1,1))*h
      endif
      IF(METH==3)THEN
      HNT(1)=H*RH
      CALLCOSET(METH,NQ,EL,TQ,MAXORD,IDOUB)
...
C------ '<--Leaving  Node %GY:'
      ENDIF
09911 CONTINUE
      IF(.NOT.(.NOT.EVALJA.or.RETCOD/=0))GOTO09978
      GOTO09990
09989 CONTINUE
C- while RETCOD==0
C------ '<--Leaving  Node %G:'
