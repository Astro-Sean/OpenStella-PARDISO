     ...
     *ubvR,LubvI,Lyman
      ...
      Parameter(Nchn=4)
      DataScatG/.False./
      If(toldg>0.d0.AND.t>toldg*1.003d0)then
      DO09997km=1,Nzon
...
09988 CONTINUE
      ...
      if(taugam(k)>30.D0.AND.incnd==0)incnd=k
09982 CONTINUE
      write(*,*)' incnd',incnd,' taugam ',taugam(max(incnd,2)),' at km='
     *,max(incnd,2)
      Rout=Homo*Ry(Nzon)
...
09973 CONTINUE
      ...
      DO09964Ip=NP,0,-1
      Kzer=Nzon
09961 IF(.NOT.(Pray(Ip)<RM(Kzer).AND.Kzer>Knth))GOTO09960
      Kzer=Kzer-1
      GOTO09961
09960 CONTINUE
...
09952 CONTINUE
     ...
     *ho(Kpl)
      If(ScatG.AND.Nstep>1)then
      fdirsumm=(chiab)
      elseif(KNadap<0)then
      ...
      K=Kpl-1
09943 IF(.NOT.(K>Kz(Ip)))GOTO09942
      chiab=HAPabW(K,Lfr)
      chiex=HAPW(K,Lfr)
...
09940 IF(.NOT.(tauz<1.d+02.AND.K<Nzon))GOTO09939
      K=K+1
      chiab=HAPabW(K,Lfr)
      chiex=HAPW(K,Lfr)
      dtau(K,Ip)=(Amu(Ip,K)*RM(K)-Amu(Ip,K-1)*RM(K-1))*chiex*rho(K)
...
09925 CONTINUE
      ...
      FJgam(k,2)=FJgam(k,2)+(FSdir(Isw,K)+FSdir(Isw+1,K))*(Amu(Isw,K)-Am
     *u(Isw+1,K))
09919 CONTINUE
