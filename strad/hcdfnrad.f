...
09989 CONTINUE
      ...
      if(Ry(K)<=Ry(K-1))then
      Badste=.true.
      write(*,*)' Difmat Km=',K-1,'  R0=',Ry(K-1),'  u0=',Uy(K-1)
      write(*,*)' Difmat Km=',K,'  R1=',Ry(K),'  u1=',Uy(K)
      Kbad=K
      Return
      endif
      endif
      Uy(K)=Fsave(Nzon+K)
      Ty(K)=Fsave(2*Nzon+K)
      If(Ty(K)<=FLOOR(4))then
      Ty(K)=FLOOR(4)
      Fsave(2*Nzon+K)=FLOOR(4)
      endif
09983 CONTINUE
      If(EVALJA)then
      CallDHapH
...
09947 CONTINUE
      ...
      Tp=Tp2
09959 CONTINUE
09941 IF(.NOT.(Km<NZON))GOTO09940
      Km=Km+1
      If(Km/=1)then
      RSm1=RS0
      else
...
09904 CONTINUE
      ...
      ET2=ET*UEI
      PT2=PT*UPI
      IF(.NOT.(Km<Ncnd))GOTO09899
      FLLF1=((R1*Tp1)**4-(R1*Tp2)**4)*Tp1up4/(CAP1*(DM1+DM2)*(Tp1up4+Tp2
     *up4))
      FLRT1=((R1*Tp1)**4-(R1*Tp2)**4)*Tp2up4/(CAP2*(DM1+DM2)*(Tp1up4+Tp2
     *up4))
      CM1INV=(Tp1up4/(CAP1*(DM1+DM2))+Tp2up4/(CAP2*(DM1+DM2)))/(Tp1up4+T
     *p2up4)
      FL1=FLLF1+FLRT1
      GOTO09897
09899 CONTINUE
      IF(.NOT.(Km==Ncnd))GOTO09898
      FLcor1=((R1*Tp1)**4-(R1*Tp2)**4)*Tp1up4/(CAP1*(DM1+DM2)*(Tp1up4+Tp
     *2up4))
      FLcor2=((R1*Tp1)**4-(R1*Tp2)**4)*Tp2up4/((CAP2*(DM1+DM2)+0.d0*RS1)
     **(Tp1up4+Tp2up4))
      CM1INV=(Tp1up4/(CAP1*(DM1+DM2))+Tp2up4/(CAP2*(DM1+DM2)+0.d0*RS1))/
     *(Tp1up4+Tp2up4)
      FL1=FLcor1+FLcor2
      FLcore=FL1
      CAPcor=CAP2
...
09910 CONTINUE
      ...
      GOTO09880
09881 CONTINUE
      FSAVE(NYDIM+Nzon+Km)=FSAVE(NYDIM+Nzon+Km)+RODM*(RTphi(Km)*U0*(-min
     *(0.d0,UQ1))/(R1-R0)**NRT-RTphi(Km+1)*U2*(-min(0.d0,UQ2))/(R2-R1)**
     *NRT)
      Tp=Tp1
      PL=PL1
      DO09878L=1,NFRUS
      ...
      hapH(L)=(Tp1up4+Tp2up4)/((Tp2up4/hapL2(L))*(1.d0+1.5d0*(exp(-tau2(
     *L))+tau2(L)-1.d0)*exp(-tau1(L)))+(Tp1up4/hapL1(L))*(1.d0+1.5d0*(ex
     *p(-tau1(L))+tau1(L)-1.d0)*exp(-tau2(L))))
      hapH1(L)=hapH(L)
09878 CONTINUE
      FRST=.TRUE.
      calltraneq
      ...
      IF(Km<NZON)THEN
      DO09875L=1,NFRUS
      FIMP=FIMP+FSAVE(NVARS*NZON-Ncnd+KRAD+(NZON-Ncnd)*(L-1)+Km)*WEIGHT(
     *L)*hapH1(L)
      FLUM=FLUM+FSAVE(NVARS*NZON-Ncnd+KRAD+(NZON-Ncnd)*(L-1)+Km)*WEIGHT(
     *L)
      If(L==LfrDebug.AND.Km<Ncnd+5)WRITE(4,'(A,2I5,1X,6(A,1P,E20.12))')'
     * km, Nperturb =',km,Nperturb,' @FH1=',FSAVE(NVARS*NZON-Ncnd+KRAD+(
     *NZON-Ncnd)*(L-1)+Km)
09875 CONTINUE
      ELSE
      DO09872L=1,NFRUS
      FIMP=FIMP+FSAVE(NVARS*NZON-Ncnd+(NZON-Ncnd)*(L-1)+Km)*HEDD(L)*WEIG
     *HT(L)*hapL1(L)
      FLUM=FLUM+FSAVE(NVARS*NZON-Ncnd+KRAD+(NZON-Ncnd)*(L-1)+Km)*WEIGHT(
     *L)*HEDD(L)
09872 CONTINUE
...
09869 CONTINUE
      HEATRA=HEATRA/ET1
09880 CONTINUE
      If(abs(KNadap)==5)FSAVE(NYDIM+Nzon+Km)=0.d0
      If(Tp1>1.D+1/UTP)then
      ...
      LfrDebug=56
      IF(Km>Ncnd.AND.Km<Ncnd+4)then
      WRITE(4,'(3(A,I10))')' LfrDebug=',LfrDebug,' Nperturb=',Nperturb
      WRITE(4,'(4(A,I10),2(A,L5))')' NSTEP=',NSTEP,' Km=',Km,' Ncnd=',Nc
     *nd,' Nfrus=',Nfrus,'  EVJA=',EVALJA,'  LTH(40)=',LTHICK(40,Km)
      WRITE(4,*)' Tdot=',FSAVE(NYDIM+2*Nzon+Km)
      WRITE(4,'(1X,6(A,1P,E10.2))')'   R1=',R1,'   U1=',U1,'  Tp=',Tp1,'
     *  PL=',PL1,'  Fl0=',fl0,'   FJ1=',FSAVE(NVARS*NZON-Ncnd+(NZON-Ncnd
     *)*(Lfrdebug-1)+Km),'   FH1=',FSAVE(NVARS*NZON-Ncnd+(NZON-Ncnd)*(Lf
     *rdebug-1)+Km+Krad),'   EDDJ(Km,LfrDebug)=',EDDJ(Km,LfrDebug),'   E
     *DDJ(Km+1,LfrDebug)=',EDDJ(Km+1,LfrDebug),'   HEDD(LfrDebug)=',HEDD
     *(LfrDebug)
      endif
      endif
