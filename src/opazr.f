     ...
     *l,YZMT,YZMPl
      ...
      CapT=XeT*(Wcap+CTOMP)+(Ccaps*Xe*ScapT+CcapZ*(ZMean+2.D0)*(2.D0*ZMT
     **YZMean+(ZMean+2.D0)*YZMT))*Pl/(Tp**2*Tpsqrt)-3.5D0*(Wcap*Xe+ZCap)
     */Tp
      If(Tp<TpDust)CapT=CapT-2.d0*DustK*(TpDust-Tp)
      CapPl=XePl*(Wcap+CTOMP)+(Ccaps*Xe*ScapPl+CcapZ*(ZMean+2.D0)*(2.D0*
     *ZMPl*YZMean+(ZMean+2.D0)*YZMPl))*Pl/(Tp**2*Tpsqrt)+(Wcap*Xe+ZCap)
      return
      end
