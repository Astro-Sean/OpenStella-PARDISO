     ...
     *,ULGEPS
      Parameter(epsNi=3.9656224881D-02,epsCo=7.0232694190D-03,epspst=2.3
     *346047930D-04,tNi=8.8000000000D+00,tCo=1.1130000000D+02)
      ...
      Kmcor=1
      Ivol=1
09999 IF(.NOT.(Am(Kmcor)<(ak-3.)/ak*Amout))GOTO09998
      Kmcor=Kmcor+1
      GOTO09999
09998 CONTINUE
      ...
      If(KNadap>0)then
      tday=T*UTIME/8.64D4
      ENGgam=XNi*(epsNi*EXP(-tday/tNi)+epsCo*(EXP(-tday/tCo)-EXP(-tday/t
     *Ni)))
      GammaJ=FJgam(Kmarg,2)
      if(tnewg>toldg)GammaJ=GammaJ+(FJgam(Kmarg,2)-FJgam(Kmarg,1))*(t-tn
     *ewg)/(tnewg-toldg)
      ENG=ENGgam*GammaJ
     ...
     *)
      ...
      IF((AM(Kmarg)-AMini)*UM<=AMHT.AND.t*Utime>tbeght.AND.t*Utime<=tbur
     *st+tbeght)THEN
      Eng=Eng+Eburst*1.D+38/(AMHT*CMS*tburst)
      ELSEIF(Kmarg>1)then
      if((AM(Kmarg)-AMini)*UM>AMHT.AND.(AM(Kmarg-1)-AMini)*UM<AMHT.AND.t
     **Utime<=tburst)THEN
      Eng=Eng+Eburst*1.D+38/(AMHT*CMS*tburst)*(AMHT-(AM(Kmarg-1)-AMini)*
     *UM)/(Dm(Kmarg)*UM)
      endif
      ENDIF
      ENDIF
