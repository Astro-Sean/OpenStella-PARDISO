     ...
     *l,YZMT,YZMPl
      ...
      DimensionYal(Natom),Yaldz(Natom),al(Natom),aldT(Natom),sigYal(Nato
     *m),AD1(Natom),AD2(Natom),AR1(Natom),AR2(Natom)
      Parameter(Nstagez=2)
      DimensionXion(Nstagez,Natom)
      LogicalLOW,HIGH
      COMMON/NIT/Xelow,Xion,Nit,LOW,HIGH
      Parameter(CPSI=3.0176945495D-04,CPRESS=1.0035847556D-04,CRGAS=8.31
     *41678120D-01,CAvD=6.0220450000D+17,CSaha=1.5270993016D+23,DLOW=1.0
     *000000000D+05,DUP=1.0000000000D+08,CWW=1.4476482730D-01,CLOW=1.666
     *6666667D+00,PIONUP=6.3151191931D+00)
      Parameter(Zf=1.2000000000D+01,Tpzb=3.8868074375D+00,Tpzf=1.4821399
     *533D+02)
      ...
      DATAPION/1.578025D+00,0.000000D+00,2.853397D+00,6.315119D+00,1.306
     *704D+00,2.829607D+00,1.686647D+00,3.435147D+00,1.580346D+00,4.0751
     *54D+00,2.502467D+00,4.753573D+00,5.963725D-01,5.487462D+00,8.87305
     *7D-01,1.744787D+00,6.946654D-01,2.184958D+00,9.460262D-01,1.896926
     *D+00,1.828806D+00,3.206300D+00,5.037659D-01,3.670029D+00,7.094036D
     *-01,1.377610D+00,9.167560D-01,1.878537D+00,8.862941D-01,2.108423D+
     *00/
      DATAPsum/1.578025D+00,1.578025D+00,2.853397D+00,9.168516D+00,1.306
     *704D+00,4.136311D+00,1.686647D+00,5.121794D+00,1.580346D+00,5.6555
     *00D+00,2.502467D+00,7.256039D+00,5.963725D-01,6.083835D+00,8.87305
     *7D-01,2.632093D+00,6.946654D-01,2.879624D+00,9.460262D-01,2.842953
     *D+00,1.828806D+00,5.035106D+00,5.037659D-01,4.173795D+00,7.094036D
     *-01,2.087014D+00,9.167560D-01,2.795293D+00,8.862941D-01,2.994717D+
     *00/
      DATAGWL/-6.931472D-01,0.000000D+00,6.931472D-01,-6.931472D-01,-4.0
     *54651D-01,-1.791759D+00,8.109302D-01,-4.054651D-01,-8.109302D-01,8
     *.109302D-01,1.791759D+00,4.054651D-01,-6.931472D-01,1.791759D+00,6
     *.931472D-01,-6.931472D-01,-1.791759D+00,6.931472D-01,-4.054651D-01
     *,-1.791759D+00,1.791759D+00,4.054651D-01,-6.931472D-01,1.791759D+0
     *0,6.931472D-01,-6.931472D-01,1.823216D-01,-1.823216D-01,-7.419373e
     *-01,7.419373D-01/
      DATARGWR/2.000000D+00,1.000000D+00,5.000000D-01,2.000000D+00,1.500
     *000D+00,6.000000D+00,4.444444D-01,1.500000D+00,2.250000D+00,4.4444
     *44D-01,1.666667D-01,6.666667D-01,2.000000D+00,1.666667D-01,5.00000
     *0D-01,2.000000D+00,6.000000D+00,5.000000D-01,1.500000D+00,6.000000
     *D+00,1.666667D-01,6.666667D-01,2.000000D+00,1.666667D-01,5.000000D
     *-01,2.000000D+00,8.333333D-01,1.200000D+00,2.100000d+00,4.761905d-
     *01/
      DataChames/'H ','He','C ','N ','O ','Ne','Na','Mg','Al','Si','Ar',
     *'K ','Ca','Fe','Ni'/
      DataAD2(1)/0.d0/
      GOTO09993
09999 CONTINUE
...
09981 CONTINUE
      ...
      RXeold=0.d0
09978 IF(.NOT.(ABS(Rxeold-Rxe)>epsurs*Rxe.AND.NIT<64))GOTO09977
      RXeold=RXe
      AR1(1)=AD1(1)*Rxe
...
09977 CONTINUE
      ...
      Write(6,*)' Tp=',Tp,'  Pl=',Pl,'   Rxeold=',Rxeold,'   Rxe=',Rxe,'
     *  epsurs=',epsurs
      STOP
      endif
      Xe=1.d0/Rxe
...
09969 CONTINUE
      ...
      Xion(1,2)=Ar1(2)*al(2)
      Xion(2,2)=Ar2(2)*al(2)
      Scap=Yal(1)*Ar1(1)*Rgwr(1,1)+Yal(2)*(Ar1(2)*Rgwr(1,2)+Ar2(2)*Rgwr(
     *2,2)*4.d0)
      ScapZZ=0.D0
      DO09966i=3,Natom
      ScapZZ=ScapZZ+Yal(i)*(1.d0-ZMean/Zf)*(Ar1(i)*Rgwr(1,i)+Ar2(i)*Rgwr
     *(2,i)*4.d0)
      Xion(1,i)=Ar1(i)*al(i)
      Xion(2,i)=Ar2(i)*al(i)
09966 CONTINUE
...
09954 CONTINUE
      ...
      ScapPl=0.D0
      DO09951i=1,Natom
      DET=DET+Yat(i)*(AD1(i)*RXe)*Psum(1,i)*((1.5D0+Psum(1,i)*RTp)*RTp-X
     *eTl)
      DEPl=DEPl+Yat(i)*(AD1(i)*RXe)*Psum(1,i)
      ScapT=ScapT+Yat(i)*(AD1(i)*RXe)*RGWR(1,i)*((1.5D0+Psum(1,i)*RTp)*R
     *Tp-XeTl)
...
09951 CONTINUE
      ...
      aldT(1)=alpT
      DO09948i=2,Natom
      alpT=(((3.D0+Psum(2,i)*RTp)*AR2(i))+((1.5D0+Psum(1,i)*RTp)*AR1(i))
     *)*RTp
      dzT=(2.D0*((3.D0+Psum(2,i)*RTp)*AR2(i))+((1.5D0+Psum(1,i)*RTp)*AR1
     *(i)))*RTp
...
09948 CONTINUE
      ...
      ScapT=Yal(1)*RGWR(1,1)*(aldT(1)-AR1(1)*((aldT(1)-XeTl*AR1(1))*al(1
     *)+XeTl))
      ScapPl=Yal(1)*RGWR(1,1)*AR1(1)*(AR1(1)*al(1)-1.D0)
      DO09945i=2,Natom
      sigmT=(Psum(2,i)*AR2(i))*((3.D0+Psum(2,i)*RTp)*RTp-2.d0*XeTl)+(Psu
     *m(1,i)*AR1(i))*((1.5D0+Psum(1,i)*RTp)*RTp-XeTl)
      ScapT=ScapT+Yal(i)*((((3.D0+Psum(2,i)*RTp)*AR2(i))*RGWR(2,i)*4.D0+
     *((1.5D0+Psum(1,i)*RTp)*AR1(i))*RGWR(1,i))*RTp-(Ar1(i)*Rgwr(1,i)+Ar
     *2(i)*Rgwr(2,i)*4.d0)*(aldT(i)-XeTl*(2.D0*AR2(i)+AR1(i)))*al(i)-XeT
     *l*(Ar1(i)*Rgwr(1,i)+Ar2(i)*Rgwr(2,i)*8.d0))
      DET=DET+Yal(i)*sigmT-sigYal(i)*(aldT(i)-XeTl*(2.D0*AR2(i)+AR1(i)))
     **al(i)
      signe=2.d0*AR2(i)*Psum(2,i)+AR1(i)*Psum(1,i)
      DEPl=DEPl+Yal(i)*signe-sigYal(i)*(2.D0*AR2(i)+AR1(i))*al(i)
      ScapPl=ScapPl+Yal(i)*((2.D0*AR2(i)+AR1(i))*al(i)*(Ar1(i)*Rgwr(1,i)
     *+Ar2(i)*Rgwr(2,i)*4.d0)-(Ar1(i)*Rgwr(1,i)+Ar2(i)*Rgwr(2,i)*8.d0))
09945 CONTINUE
      ET=ET+CRgas*(1.5D0*(XYZA+Xe+Tp*XeT)+DET)
      EPl=EPl+CRgas*(1.5D0*Tp*XePl-DePl*(XePl*RXe+1.d0))
...
09998 CONTINUE
      ...
      Egas=1.5D0*P*RPl
      If(LST/=0)Then
      PPl=(5.D0*tret)*CPRESS*Ye*dY+CRgas*(XYZA+Ye/(1.D0+0.4D0*PSI))*Tp-C
     *Rgas*Ye/(1.D0+0.4D0*PSI)**2*(0.4D0*tret2)*PSI*Tp
      PT=CRgas*(XYZA+Ye/(1.D0+0.4D0*PSI))*Pl+CRgas*Ye/(1.D0+0.4D0*PSI)**
     *2*0.4D0*PSI*Pl
      EPl=1.5D0*PPl-Egas
      ET=1.5D0*PT*RPl
      ZMT=0.D0
...
09994 CONTINUE
      IF(LST/=0)THEN
      PT=PTdeg*WP+PT*(1.D0-WP)+(ETdeg-ET)*Pl**2*(12.D0*WW**2*(1.D0-WW)*W
     *D)
      ET=ETdeg*WP+ET*(1.D0-WP)
      PPl=PPLdeg*WP+PPl*(1.D0-WP)+(Pdeg-P)*(12.D0*WW**2*(1.D0-WW)*WD)+(E
     *PLdeg-EPl)*Pl*(12.D0*WW**2*(1.D0-WW)*WD)+(Edeg-Egas)*(Pl**2*12.D0*
     *WW*(2.D0-3.D0*WW)*WD**2-12.D0*CWW*WW**2*(1.D0-WW)+24.D0*Pl*WW**2*(
     *1.D0-WW)*WD)
      EPl=EPLdeg*WP+EPl*(1.D0-WP)+(Edeg-Egas)*(Pl*12.D0*WW**2*(1.D0-WW)*
     *WD)
